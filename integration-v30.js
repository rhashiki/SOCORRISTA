/* Re.Force APH — Integration bootstrap v95 (legacy filename) */
window.RF_BUILD=95;
window.__RF_BOOT_ERRORS=window.__RF_BOOT_ERRORS||[];
(function(){
  const start=document.querySelector('#startShift');if(start){start.disabled=true;start.textContent='OTIMIZANDO FPS...';}
  const meta=document.querySelector('meta[name="reforce-build"]');if(meta)meta.content='95';
  const modeBuild=document.querySelector('.mode-row span:last-child');if(modeBuild)modeBuild.textContent='BUILD 95';
  const modeView=document.querySelector('.mode-row span:first-child');if(modeView)modeView.textContent='PRIMEIRA PESSOA';
  const badge=document.createElement('div');badge.textContent='BUILD 95';badge.style.cssText='position:fixed;right:8px;bottom:8px;z-index:9999;padding:4px 7px;background:#090b0dcc;border:1px solid #ffffff22;color:#d8a05b;font:700 9px/1.2 system-ui;letter-spacing:.08em;pointer-events:none';
  ...
  function audit(){
    const required=[
      ...
      'v95PixelCap','v95ApplyBudget','v95TargetFPS','v95FarPlane'
    ];
    ...
    if(missing.length||bootErrors.length){console.warn('Re.Force v95 integration warning',{missing,bootErrors});const t=document.querySelector('#toast');if(t){t.hidden=false;t.textContent='Aviso de integração detectado. Recarregue a página; se persistir, reporte a etapa em que parou.';setTimeout(()=>t.hidden=true,5500);}}
    else console.info('Re.Force APH Build 95 FPS otimizada integrada com sucesso.');
  }
  window.addEventListener('load',()=>{if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw.js?v=95').catch(err=>console.warn('SW v95:',err));audit();},{once:true});
})();
