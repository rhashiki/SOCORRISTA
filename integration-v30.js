/* Re.Force APH — Integration bootstrap v45 (legacy filename) */
window.RF_BUILD=45;
window.__RF_BOOT_ERRORS=window.__RF_BOOT_ERRORS||[];
(function(){
  const badge=document.createElement('div');badge.textContent='BUILD 45';badge.style.cssText='position:fixed;right:8px;bottom:8px;z-index:9999;padding:4px 7px;background:#090b0dcc;border:1px solid #ffffff22;color:#d8a05b;font:700 9px/1.2 system-ui;letter-spacing:.08em;pointer-events:none';
  document.addEventListener('DOMContentLoaded',()=>document.body.appendChild(badge),{once:true});
  window.addEventListener('load',()=>{
    if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw.js?v=45').catch(err=>console.warn('SW v45:',err));
    const required=[
    ...
    if(missing.length||bootErrors.length){
      console.warn('Re.Force v45 integration warning',{missing,bootErrors});
    ...
    }else console.info('Re.Force APH Build 45 integrada com sucesso.');
  },{once:true});
})();
