/* Re.Force APH — Integration bootstrap v40 (legacy filename) */
window.RF_BUILD=40;
window.__RF_BOOT_ERRORS=window.__RF_BOOT_ERRORS||[];
(function(){
  const badge=document.createElement('div');
  badge.textContent='BUILD 40';
  ...
  window.addEventListener('load',()=>{
    if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw.js?v=40').catch(err=>console.warn('SW v40:',err));
    const required=[
      ...
      'v31Texture','v32UpdatePatientVisual','v33ApplyCorridor','v34Announce','v35Hash',
      'v36SetupMap','v37ResolveCamera','v38Apply','v39CreateStretcher','v40SpawnTraffic'
    ];
    ...
    if(missing.length||bootErrors.length){
      console.warn('Re.Force v40 integration warning',{missing,bootErrors});
      const t=document.querySelector('#toast');
      if(t){t.hidden=false;t.textContent='Aviso de integração detectado. Recarregue a página; se persistir, reporte a etapa em que parou.';setTimeout(()=>t.hidden=true,5500);}
    }else console.info('Re.Force APH Build 40 integrada com sucesso.');
  },{once:true});
})();
