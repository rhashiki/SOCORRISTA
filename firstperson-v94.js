...
function v94SetDir(yaw,pitch,out=V94_CAM_DIR){const cp=Math.cos(pitch);out.set(-Math.sin(yaw)*cp,Math.sin(pitch),-Math.cos(yaw)*cp);return out;}
function v94PointInBuilding(p){if(typeof v37PointHitsBuilding==='function')return v37PointHitsBuilding(p);return buildings.some(b=>Math.abs(p.x-b.x)<b.w/2+.2&&Math.abs(p.z-b.z)<b.d/2+.2&&p.y<23);}
function v94SafeVehicleCamera(out){
  ...
  const centre=ambulance.root.position;const start=V94_FORWARD.copy(centre).setY(1.72),desired=V94_RIGHT.copy(out);
  for(let i=1;i<=8;i++){const t=i/8;out.lerpVectors(start,desired,t);if(!v94PointInBuilding(out))return out;}
  out.copy(centre).setY(2.15);return out;
}
function v94ApplyCameraProjection(fov){if(!camera)return;const far=V94_MOBILE?148:205;let changed=false;if(Math.abs(camera.fov-fov)>.05){camera.fov=fov;changed=true;}if(camera.near!==.055){camera.near=.055;changed=true;}if(camera.far!==far){camera.far=far;changed=true;}if(changed)camera.updateProjectionMatrix();}
...
updateCamera=function(dt){
  ...
  if(controlled==='vehicle'&&ambulance?.root){
    ...
    V94_YAW=-ambulance.heading+V94_VEH_OFFSET;v94SafeVehicleCamera(V94_CAM_POS);camera.position.copy(V94_CAM_POS);
...
function v94AimAt(world){if(!player?.root||!world)return;const eye=V94_CAM_POS.copy(player.root.position).setY(player.root.position.y+1.63),d=V94_CAM_DIR.copy(world).sub(eye).normalize();V94_YAW=Math.atan2(-d.x,-d.z);V94_PITCH=Math.asin(clamp(d.y,-1,1));}
...
function v94SetupReticle(){if(document.querySelector('#fpsReticle'))return;const r=document.createElement('div');r.id='fpsReticle';r.setAttribute('aria-hidden','true');document.querySelector('#game')?.appendChild(r);document.body.classList.add('rf-fps');}
...
init=async function(){V94_YAW=.15;V94_PITCH=0;V94_VEH_OFFSET=0;V94_LAST_LOOK=0;await v94BaseInit();if(player?.visual)player.visual.visible=false;if(player?.root?.userData?.v58Shadow)player.root.userData.v58Shadow.visible=false;if(typeof v93ApplyPerformanceBudget==='function')v93ApplyPerformanceBudget();v94SetupReticle();v94InstallLookInput();v54ApplyLOD?.();updateCamera(1/60);};
v94SetupReticle();
